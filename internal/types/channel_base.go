// Package types provides the base types for channels in the gnyx package.
package types
...
type ChannelBase[T any] struct {
	*gl.LoggerZ              // Logger for this ChannelBase instance
...
func NewChannelBase[T any](name string, buffers int, logger *gl.LoggerZ) ci.IChannelBase[any] {
	if logger == nil {
		logger = gl.GetLoggerZ("ChannelBase")
	}
	mu := NewMutexesType()
	if buffers <= 0 {
		buffers = lgBuf
