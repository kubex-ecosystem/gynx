...
type TrainingLessonsController struct {
	service svc.Service[mdl.TrainingBadges]
}

func NewTrainingLessonsController(service svc.Service[mdl.TrainingLessons]) Controller[mdl.TrainingLessons] {
	return NewController[mdl.TrainingLessons](nil, service)
}
...
// 			c.JSON(http.StatusNotFound, gin.H{"error": "TrainingLessons not found"})
// 			return
// 		}
// 		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
// 		return
...
// 	var payload mdl.TrainingLessons
