...
kill_old_be() {
  ...
  if test -f "${KUBEX_BE_PID_FILE_PATH:-}"; then
    rm -f "${KUBEX_BE_PID_FILE_PATH:-}" 2>/dev/null || true
  fi
  return 0
}
...
create_log_files() {
  ...
  touch "${KUBEX_BE_LOG_FILE_PATH?Error: KUBEX_BE_LOG_FILE_PATH is not set}" 2>/dev/null || true

  # Create deploy log file if it doesn't exist
  touch "${KUBEX_BE_DEPLOY_LOG_PATH?Error: KUBEX_BE_DEPLOY_LOG_PATH is not set}" 2>/dev/null || true

  # Log the start attempt
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting Kubex BE process..." >> "$KUBEX_BE_LOG_FILE_PATH"
}

set_exec_flags() {
  if test -n "${_ENV_FILE:-}"; then
    _KUBEX_BE_ENV_FILE="-E ${_ENV_FILE:-}"
  fi

  if test -n "${KUBEX_BE_BIND:-}"; then
    _KUBEX_BE_BIND="-b $KUBEX_BE_BIND"
  else
    _KUBEX_BE_BIND=""
  fi

  if test -n "${KUBEX_BE_PORT:-}"; then
    _KUBEX_BE_PORT="-p $KUBEX_BE_PORT"
  else
    _KUBEX_BE_PORT="-p 4000"
  fi

  if test -n "${KUBEX_BE_ENV_MODE:-}"; then
    _KUBEX_BE_ENV_MODE="-D"
  else
    _KUBEX_BE_ENV_MODE=""
  fi
}
...
start_backend() {
  ...
  if ! test -f "${KUBEX_BE_CONFIG_PATH:-}"; then
    echo "Kubex BE configuration file not found at path: ${KUBEX_BE_CONFIG_PATH:-}"
    return 1
  fi
  ...
  nohup "${_exec_cmd[@]}" >> "${KUBEX_BE_LOG_FILE_PATH:-}" 2>&1 &

  # Get the PID and exit code of the started process
  KUBEX_BE_PROCESS_PID=$!
  KUBEX_BE_PROCESS_EXIT_CODE=$?

  sleep 0.5

  if ! test -n "$KUBEX_BE_PROCESS_PID" || ! kill -0 "$KUBEX_BE_PROCESS_PID" 2>/dev/null; then
    echo "Failed to start Kubex BE process. Check the log file at: ${KUBEX_BE_LOG_FILE_PATH:-}"
    return 1
  fi

  # Save the PID to a file and disown the process
  echo $KUBEX_BE_PROCESS_PID > "${KUBEX_BE_PID_FILE_PATH:-}"
  disown $KUBEX_BE_PROCESS_PID

  sleep 0.5

  # Health check: Verify if the process is still running
  if ! kill -0 "$KUBEX_BE_PROCESS_PID" 2>/dev/null; then
    echo "Kubex BE process failed to start. Check the log file at: ${KUBEX_BE_LOG_FILE_PATH:-}"
    return 1
  fi
  ...
  Kubex BE PID: $KUBEX_BE_PROCESS_PID
  Bind Address: $KUBEX_BE_BIND
  Port: $KUBEX_BE_PORT
  Log File: $KUBEX_BE_LOG_FILE_PATH
  ------------------------------"

  # Log the start info
  echo -e "$_LOG_HEADER" >> "$KUBEX_BE_DEPLOY_LOG_PATH"
}
...
exit $KUBEX_BE_PROCESS_EXIT_CODE
